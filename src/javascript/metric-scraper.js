...
import { DEFAULT_OTLP_ENDPOINT, OtlpClient, OtlpLogger } from "shared/otlpTelemetry";
...
export async function main(ns) {
    ...
    const log = new OtlpLogger(ns, { endpoint, level: flags.logLevel });
    ...
    const base = { bitnode: ns.getResetInfo().currentNode };
...
function scrapePlayer(ns, otel, base) {
 ...
 */
function scrapeIncome(ns, otel, base, prev, log) {
    ...
    if (emitted.length > 0)
        log.log(`income: ${emitted.join(", ")}`, LogLevel.VERBOSE);
    return cur;
}
/** Reputation with every joined faction. Caller guards on Singularity availability. */
function scrapeFactionRep(ns, otel, base) {
 ...
 */
function scrapeGang(ns, otel, base, hasSingularity) {
    let inGang = false;
    try {
    ...
    const attrs = { ...base, faction: gang.faction };
